...
export default function FormLaporan() {
  ...
  const handleSubmit = async () => {
    if (!form.jenis) { showToast("Pilih jenis masalah terlebih dahulu"); return; }
    if (!form.cerita.trim()) { showToast("Tolong ceritakan masalahmu"); return; }
    ...
    try {
      ...
      const response = await fetch("/api/laporan", {
        ...
        body: JSON.stringify({
          ...
          nim: form.nim,
          email: form.email,
          jenis_masalah: form.jenis,
          tingkat_keparahan: form.level,
          deskripsi: form.cerita,
        }),
      });
    ...
    <>
      <section id="lapor" className="px-[5%] py-16">
        ...
        <div className="text-gray-400 text-[15px] mb-10">
          Ceritakan masalahmu — kami akan membantu menghubungkanmu dengan dukungan yang tepat
        </div>
        ...
        <div className="bg-white border border-black/10 rounded-2xl p-10 max-w-2xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
            <div>
              ...
              <input type="text" id="f_nama" placeholder="Nama kamu..." value={form.nama} onChange={handleChange} className={inputClass} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">NIM (opsional)</label>
              <input type="text" id="f_nim" placeholder="Nomor Induk Mahasiswa" value={form.nim} onChange={handleChange} className={inputClass} />
            </div>
          </div>
          ...
          <div className="mb-5">
            ...
            <select id="f_jenis" value={form.jenis} onChange={handleChange} className={inputClass}>
              <option value="">— Pilih kategori —</option>
              <option value="Stres Akademik">Stres Akademik</option>
              <option value="Kecemasan & Panik">Kecemasan &amp; Panik</option>
              <option value="Burnout">Burnout</option>
              <option value="Masalah Finansial">Masalah Finansial</option>
              <option value="Kesepian & Isolasi">Kesepian &amp; Isolasi</option>
          ...
          <div className="mb-5">
            ...
            <select id="f_level" value={form.level} onChange={handleChange} className={inputClass}>
              <option value="">— Pilih tingkat —</option>
              <option value="Ringan">Ringan — masih bisa diatasi sendiri</option>
              <option value="Sedang">Sedang — perlu dukungan dari orang lain</option>
              <option value="Berat">Berat — sangat mengganggu aktivitas</option>
          ...
          {/* Cerita */}
          <div className="mb-5">
            <label className="block text-sm font-medium mb-2">Ceritakan Masalahmu</label>
            <textarea
              id="f_cerita"
            ...
            <div className="text-[12px] text-gray-400 mt-1.5">
              Laporan ini bersifat anonim dan hanya dapat diakses oleh konselor kampus.
            </div>
          </div>

          {/* Email */}
          <div className="mb-6">
            <label className="block text-sm font-medium mb-2">Email (untuk tindak lanjut)</label>
            <input type="email" id="f_email" placeholder="email@students.umsu.ac.id" value={form.email} onChange={handleChange} className={inputClass} />
            <div className="text-[12px] text-gray-400 mt-1.5">
              Opsional — isi jika ingin dihubungi kembali oleh tim konselor.
            </div>
          </div>
